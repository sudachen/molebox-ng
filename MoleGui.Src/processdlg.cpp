...
struct ProcessDlg: wxDialog
{
    ...
    ProcessDlg(wxWindow* parent)
        : wxDialog(parent,wxID_ANY,Tr(104,"Packing..."),wxDefaultPosition,wxDefaultSize,wxCAPTION),
    ...
    void CreateControls()
    {
        ...
        buttons->Add( go_test_button_ = new wxButton(this,ctID_GOTEST,Tr(105,"Go Test")), 0, wxRIGHT, 10 );
        buttons->Add( break_button_ = new wxButton(this,ctID_BREAK,Tr(106,"Break")), 0, 0, 0 );
    ...
    void OnGoTest(wxCommandEvent& event)
    {
        if ( intercom->Finished() != -1 && g_mxf_is_executable )
        {
            ...
            if ( !SysExec(_S*L"\"%s\""%target,ehf) )
                DisplayError(this,_S*Tr(107,"Could not run packed executable\n\n-Reason-\n%s")%ehf.Str(),L"Error");
    ...
    void OnTimer(wxTimerEvent& event)
    {
        __lockon__(intercom->lock_)
        {
            ...
            {
                break_button_->SetLabel(Tr(108,"Close"));
                if ( intercom->Finished() != -1 && g_mxf_is_executable )
    ...
    static void Coroutine(ProcDlgIntercom* intercom)
    {
            ...
            //if ( SysPipe(0|_S*L"%s "%appname,&childin,&childout,&child,true,false,ehf) )
        {
            ...
            while ( !pot.Error() )
            {
                ...
                if ( intercom->kill_ )
                {
                    ...
                    i->AppendText(Tr(109,"terminated"));
                    i->Finish(-1);
                    goto e;
                }
            }
            i->AppendText(Tr(110,"finished"));
        ...
        {
            i->AppendText(Tr(111,"failed to start mxbpack.exe"));
    ...
    bool DoDialog()
    {
        CreateControls();
        timer_.Start(100);
        intercom->AppendText(Tr(112,"Starting packaging process ..."));
