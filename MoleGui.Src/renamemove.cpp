...
struct RenameMoveDlg : wxDialog
{
    ...
    RenameMoveDlg(wxWindow* parent, bool operate_folder, bool enable_rename, StringW& newfold, StringW& name)
        : wxDialog(parent,wxID_ANY,
                   (operate_folder?Tr(121,"Move/Rename Folder"):Tr(122,"Move/Rename File")),
    ...
    wxPanel* MakeSubControls(wxWindow* parent)
    {
        ...
        if ( operate_folder_ )
        {
            topszr->Add( new wxStaticText(pan,wxID_ANY,
                                          (enable_rename_?Tr(123,"Move/Rename folder:"):Tr(124,"Move folder:")))
                         , 0, wxBOTTOM|wxTOP , 5 );
        ...
        {
            topszr->Add( new wxStaticText(pan,wxID_ANY,
                                          (enable_rename_?Tr(125,"Move/Rename file:"):Tr(126,"Move file:")))
                         , 0, wxBOTTOM|wxTOP , 5 );
            topszr->Add( new wxStaticText(pan,wxID_ANY,+name_,
                                          wxDefaultPosition, wxDefaultSize, wxSTATIC_BORDER),
                         0, wxBOTTOM|wxGROW , 5 );
        ...
        if ( enable_rename_ )
        {
            topszr->Add( new wxStaticText(pan,wxID_ANY,Tr(127,"With new name:")), 0, wxBOTTOM|wxTOP , 5 );
        ...
        if ( !operate_folder_ && !name_ )
            topszr->Add( new wxStaticText(pan,wxID_ANY,Tr(128,"Move selected files to subfolder:")), 0, wxBOTTOM|wxTOP , 5 );
        else if ( enable_rename_ )
            topszr->Add( new wxStaticText(pan,wxID_ANY,Tr(129,"To/At subfolder:")), 0, wxBOTTOM|wxTOP , 5 );
        else
            topszr->Add( new wxStaticText(pan,wxID_ANY,Tr(130,"To subfolder:")), 0, wxBOTTOM|wxTOP , 5 );
